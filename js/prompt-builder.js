...
export function buildPrompt(state) {
  ...
  // En js/prompt-builder.js, dentro de la función buildPrompt(state):

	// --- Language Consistency Constraint ---
	lines.push('## Language & Typography Consistency');
	lines.push('- DUAL-LANGUAGE RULE: Detect the language used in the "Topic / Lesson Title" and "Learning Objective".');
	lines.push('- ALL visible text elements inside the graphic (main titles, section headers, diagrams, callout labels, step-by-step descriptions, and captions) MUST strictly use THAT SAME SINGLE LANGUAGE throughout the entire image.');
	lines.push('- Do NOT mix languages. If the topic is in Spanish, translate all visual labels and step descriptions to Spanish. If in English, render everything in English.');
	lines.push('');
